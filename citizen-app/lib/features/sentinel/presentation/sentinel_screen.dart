...
class _SentinelScreenState extends ConsumerState<SentinelScreen> {
  ...
  // The number on screen is a SAFETY score (high = safe), so colour and label
  // come from the model's own risk level — never from a threshold on the score.
  Color _scoreColor(RiskScore? rs) {
    if (rs == null) return AppColors.textTertiary;
    switch (rs.level) {
      case RiskLevel.high:
  ...
  String _chipLabel(RiskScore? rs) => rs?.safetyLabel ?? 'UNKNOWN';
  ...
  Widget build(BuildContext context) {
    ...
    final score = riskScore?.score;
    // Prefer the zone name the model returned; fall back to the geocoded area.
    final zoneLabel = (riskScore?.zoneName.isNotEmpty ?? false)
        ? riskScore!.zoneName.toUpperCase()
        : state.areaName.toUpperCase();
    final location = state.pincode > 0
        ? '${state.pincode} · $zoneLabel'
        : '— · ACQUIRING LOCATION';
    final scoreColor = _scoreColor(riskScore);
    final chipLabel = _chipLabel(riskScore);
                ...
                if (state.isLoading)
                  ...
                  child: Column(
                    ...
                    children: [
                              ...
                              // ── 5. "YOUR SAFETY SCORE" label ────────
                              Text(
                                lang == 'ta'
                                    ? 'உங்கள் பாதுகாப்பு மதிப்பெண் (100-ல்)'
                                    : 'YOUR SAFETY SCORE / 100',
                              ...
                              Row(
                                ...
                                children: [
                                  ...
                                  if (riskScore != null) ...[
                                    ...
                                    RkStatusChip(
                                      label: riskScore.level.name,
                                      color: scoreColor,
                                    ),
                                    const SizedBox(width: AppSpacing.sm),
                                    RkStatusChip(
                                      label:
                                          '${(riskScore.confidence * 100).toStringAsFixed(0)}% CONF',
                                      color: AppColors.accentBright,
                                    ),
                                  ],
                                ],
...
class _StatsRow extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // Both cards read straight off the model response — no placeholder counts.
    final rs = state.riskScore;
    final zone = (rs?.zoneName.isNotEmpty ?? false)
        ? rs!.zoneName
        : (state.areaName.isNotEmpty ? state.areaName : '—');
    final model = rs?.source == 'sagemaker'
        ? 'SageMaker'
        : (rs?.source ?? '—');
    ...
    return Row(
      children: [
        ...
        Expanded(
          child: _StatCard(
            label: lang == 'ta' ? 'ML மாதிரி' : 'ML MODEL',
            value: model,
            isNumeric: false,
          ),
        ),
