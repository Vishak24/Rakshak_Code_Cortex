...
class ScoreScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // `score` is a SAFETY score (0–100, high = safe). "Critical" is the model's
    // own HIGH-risk label, not a threshold on the number.
    final result = state.result;
    final score = result?.score;
    final isCritical =
        result != null && (result.level == RiskLevel.high || result.level == RiskLevel.critical);
