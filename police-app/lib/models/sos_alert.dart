class SosAlert {
  ...
  /// Straight-line km from the assigned patrol to the incident.
  final double? distanceKm;

  /// Live position of the assigned patrol, recomputed by the backend on read.
  final double? patrolLat;
  final double? patrolLng;

  /// Patrol state as the backend sees it: Responding | AtScene | Returning.
  final String? patrolStatus;

  /// Name the citizen raised the SOS under.
  ...
  /// True once the officer has reported arrival.
  bool get isReached {
    final s = status.toLowerCase();
    return s == 'reached' || s == 'on_scene' || s == 'at_scene';
  }
  ...
  factory SosAlert.fromJson(Map<String, dynamic> json) {
    ...
    return SosAlert(
      ...
      assignedVehicle: json['assigned_vehicle']?.toString(),
      distanceKm: _toDouble(json['distance_km']),
      patrolLat: _toDouble((json['patrol_position'] as Map?)?['lat']),
      patrolLng: _toDouble((json['patrol_position'] as Map?)?['lng']),
      patrolStatus: json['patrol_status']?.toString(),
      citizenName: json['username']?.toString(),
    );
  }
